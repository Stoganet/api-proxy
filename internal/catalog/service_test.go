...
func (f *fakeJF) GetItem(_ context.Context, _, _ string) (*jellyfin.Item, error) {
	return f.item, f.err
}

func (f *fakeJF) GetItems(_ context.Context, _ string, _ jellyfin.GetItemsOpts) (*jellyfin.ItemsResult, error) {
...
func TestService_GetItem_ReturnsDetail(t *testing.T) {
	...
	svc := newSvc(jf)
	d, err := svc.GetItem(context.Background(), "jf-user-1", "jf-tok", "jf-1")
	if err != nil {
...
func TestService_GetItem_PropagatesNotFound(t *testing.T) {
	...
	svc := newSvc(jf)
	_, err := svc.GetItem(context.Background(), "jf-user-1", "tok", "missing")
